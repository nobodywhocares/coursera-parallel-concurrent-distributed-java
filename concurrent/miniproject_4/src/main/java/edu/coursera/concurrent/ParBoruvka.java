...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        ParComponent primary;
        ParComponent associate;
        while (null != (primary = nodesLoaded.poll())) {
            if (null == primary || !primary.lock.tryLock()) {
                // primary node is locked so try another
                continue;
            }
            if (primary.isDead) {
                // primary node already processed
                primary.lock.unlock();
                continue;
            }
            // If no associate available then primary is solution because
            // all nodes are already merged
            Edge<ParComponent> minEdge = primary.getMinEdge();
            if (minEdge == null) {
                solution.setSolution(primary);
                // primary.lock.unlock();
                break;
            }
            // Process associate
            associate = minEdge.getOther(primary);
            if (associate.isDead) {
                // Associate node is already processed so release locks
                primary.lock.unlock();
                associate.lock.unlock();
            } else if (associate.lock.tryLock()) {
                primary.merge(associate, minEdge.weight());
                associate.isDead = true;
                nodesLoaded.add(primary);
                primary.lock.unlock();
                associate.lock.unlock();
            } else {
                nodesLoaded.add(primary);
