...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = generateNewRanks(sites, ranks);
        return newRanks
    ...
    protected static JavaPairRDD<Integer, Double> generateNewRanks(
            final JavaPairRDD<Integer, Website> sites,
            final JavaPairRDD<Integer, Double> ranks) {
        return sites.join(ranks).flatMapToPair(kv -> flattenMap(kv));
    }

    protected static Iterable<Tuple2<Integer, Double>> flattenMap(
        ...
        if (0 < edges.getNEdges()) {
            Iterator<Integer> edgeLoop = edges.edgeIterator();
            do {
                int target = edgeLoop.next();
                contribs.add(new Tuple2<>(target, rank / (double) edges.getNEdges()));
            } while (edgeLoop.hasNext());
        }
        return contribs;
    }
}
